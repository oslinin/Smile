...
| **IV** e.g. `82.4%` | The implied volatility the smile assigns to that strike: σ = 0.80 × (1 + 2.0 × ln(K/S)²). | `smileVol()` in the browser with `SIGMA_GLOBAL = 0.80`, `ALPHA = 2.0` (the pre-event flat surface; the on-chain hook's per-tenor sigma and the β skew term are not read here). |
| **Δ** e.g. `0.53` (calls) / `−0.47` (puts) | Black-Scholes delta at the smile volatility: the option's price sensitivity to a $1 move in spot, and roughly the probability of expiring in the money. Put delta is call delta minus one. | `callDelta()` with the Abramowitz–Stegun normal CDF. |
| **Bid↓sell** (green) and **Ask↑buy** (red) e.g. `$121.30` | Per-unit premium. Bid is the Black-Scholes value × 0.99; Ask is × 1.01. Ask cells are dimmed and unclickable unless the strike is inside the active range on the matching side. | `priceWAD()` in `OptionMatrix.tsx`. Note that these are the browser's Black-Scholes approximation of the on-chain quote; the price actually charged is computed by the SwapVM opcode at fill time and is shown in the buy panel. |
...
| **Active Positions** `≥1` or `0` — `Options sold · collateral locked` | Whether any collateral is locked on the range. This is a yes/no derived from used collateral, not a count. | `usedCollateral > 0`. |
