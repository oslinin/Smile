...
- **L4, one transaction can drain a whole range.** No vault imposes a per-trade or per-block size cap, so a single fill can pull an authorization's entire remaining collateral at one price. Per-range block caps were on `MarginVault`'s plan and were cut; the loss per event is bounded by the range's `maxCollateral`.
